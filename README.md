...
<p>This is a release-only, community-driven, GPL-3.0 licensed, copyleft project derived from Parity and Bitcoin.org(forked from <a href="https://github.com/paritytech/parity-bitcoin/">parity/bitcoin</a>).  Paritytech, Bitcoin.org and other third party code contributors, if any, in this repository reserves all copyrights.</p>
<h1>Contribution Credits</h1>
<p>Special Thanks to all the contributions from <strong><a href="https://github.com/paritytech/parity-bitcoin/graphs/contributors">Contributor List</a></strong>. And please checkout <strong><a href="https://github.com/paritytech/parity-bitcoin/commits/master">Commit History</a></strong> to view their work. Superstring Community values all contributions and especially appreciates those generous contributions from partiytech, Bitcoin.org and other third parties, directly or indirectly.</p>
...
# The Susy Bitcoin client.

[![Build Status][travis-image]][travis-url] [![Snap Status](https://build.snapcraft.io/badge/susytech/susy-bitcoin.svg)](https://build.snapcraft.io/user/susytech/susy-bitcoin)

Gitter [![Gitter https://gitter.im/susytech/susy-bitcoin](https://badges.gitter.im/susytech/susy-bitcoin.svg)](https://gitter.im/susytech/susy-bitcoin)
...
[travis-image]: https://travis-ci.com/susytech/susy-bitcoin.svg?token=DMFvZu71iaTbUYx9UypX&branch=master
[travis-url]: https://travis-ci.com/susytech/susy-bitcoin
[doc-url]: https://susytech.github.io/susy-bitcoin/susybtc/index.html

## Installing from source

Installing `susybtc` from source requires `rustc` and `cargo`.
...
#### Clone and build susybtc

Now let's clone `susybtc` and enter it's directory:

```
git clone https://octonion.institute/susytech/susy-bitcoin
cd susy-bitcoin
```

`susybtc` can be build in two modes. `--debug` and `--release`. Debug is the default.

```
# builds susybtc in debug mode
cargo build -p susybtc
```

```
# builds susybtc in release mode
cargo build -p susybtc --release
```

`susybtc` is now available at either `./target/debug/susybtc` or `./target/release/susybtc`.
...
```
sudo snap install susy-bitcoin --edge
```

## Running tests

`susybtc` has internal unit tests and it conforms to external integration tests.
...
```
# let's start susybtc in regtest compatible mode
./target/release/susybtc --btc --regtest
...
By default susy connects to bitcoind-seednodes. Full list is [here](./susybtc/seednodes.rs).
...
```
./target/release/susybtc --btc
```

To start syncing the testnet:

```
./target/release/susybtc --btc --testnet
```

To not print any syncing progress add `--quiet` flag:

```
./target/release/susybtc --btc --quiet
```
...
./target/release/susybtc import "$BITCOIND_DB/Bitcoin/blocks"
```

By default import verifies imported the blocks. You can disable this, by adding `--verification-level==none` flag.

```
./target/release/susybtc import "#BITCOIND_DB/Bitcoin/blocks" --btc --skip-verification
```

## Command line interface

Full list of CLI options, which is available under `susybtc --help`:

```
susybtc 0.1.0
Superstring Community <admin@superstring.ch>
Susy Bitcoin client

USAGE:
    susybtc [FLAGS] [OPTIONS] [SUBCOMMAND]
...
The Susy-bitcoin `network` interface.
...
The Susy-bitcoin `blockchain` data interface.
...
The Susy-bitcoin `miner` data interface.
...
The Susy-bitcoin `raw` data interface.
...
```
RUST_LOG=verification=info ./target/release/susybtc --btc
```

`susybtc` started with this environment variable will print all logs coming from `verification` module with verbosity `info` or higher. Available log levels are:
...
```
RUST_LOG=sync=trace,p2p=trace,verification=trace,db=trace ./target/release/susybtc --btc
```

## Internal documentation

Once released, `susybtc` documentation will be available [here][doc-url]. Meanwhile it's only possible to build it locally:

```
cd susy-bitcoin
./tools/doc.sh
open target/doc/susybtc/index.html
```
